...
/* ----------------------------------------------------------------------
   ...
   interactions when there is long range electrostatics.
   Stephen Farr (EPCC)
  ...
  group2(NULL)
{
  ...
  while (iarg < narg) {
    ...
    if (strcmp(arg[iarg],"kspace") == 0) {
      ...
      //else if (strcmp(arg[iarg+1],"offcentre") == 0) kspaceflag = 2; this is not enabled in this version
...
void ComputeInter::init_list(int /*id*/, NeighList *ptr)
{
  list = ptr;
}
...
double ComputeInter::compute_scalar()
{
...
void ComputeInter::pair_contribution()
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (molecule[i] == molecule[j]){
          ...
          // interactions are handled correctly and the full coloumb term is exlcuded
          //continue;
          factor_lj=0.0;
          factor_coul=0.0;
...
void ComputeInter::kspace_contribution()
{
...
void ComputeInter::kspace_correction()
{
